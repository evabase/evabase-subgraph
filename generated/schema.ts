...
export class TvlSummary extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save TvlSummary entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type TvlSummary must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("TvlSummary", id.toString(), this);
    }
  }

  static load(id: string): TvlSummary | null {
    return changetype<TvlSummary | null>(store.get("TvlSummary", id));
  }
